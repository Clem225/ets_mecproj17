...
TW=T_Wto %ratio poussee poids
CD0=Cd0 %coefficient de trainee a portance nulle de l'avion 
M=0.32 % Mach de montee
H=0 % altitude au debut de la montee, ft
dHdt=300 % taux de montee (ft/min) 
[ws, twmin] = ws_climb(TW, CD0, A, H, dHdt, M);

[T0, a0, P0, rho0] = atmosisa(distdim(25000,'ft','km'));
tx_montee = 300/60; %%slide 41 cours 3
angle_montee = rad2deg(asin(tx_montee/Vto));
%%angle_montee = acos(((1/2)*rho0*(Vto^2)*S*CLmaxTO)/Wto);
e = 0.8;
k = 1/(pi*A*e);


tx_montee_valid = Vto*((T_Wto)-(1/2)*rho0*Vto^2*((ws)^-1)*Cd0-(ws)*(k*((cos(deg2rad(angle_montee)))^2)/(S*(1/2)*rho0*Vto^2)));
% ITERATION POUR TROUVER LE BON TAUX DE MONTEE

temps_montee = 25000/tx_montee;
